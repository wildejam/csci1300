...
// Checks if the player is near a merchant, i.e, a merchant is directly up, down, left, or right of player. Returns true if so.
bool Map::isNearMerchant(){
    // Checks used to see if player is near a merchant horizontally or vertically
    bool horizontalCheck = false;
    bool verticalCheck = false;

    // Check every single merchant and see if any of them are close to the player
    for (int i = 0; i < num_merchants; i++) {
        if (merchantPositions[i][0] == playerPosition[0] - 1 || merchantPositions[i][0] == playerPosition[0] + 1) {
            horizontalCheck = true;
        }
        if (merchantPositions[i][1] == playerPosition[1] - 1 || merchantPositions[i][1] == playerPosition[1] + 1) {
            verticalCheck = true;
        }
    }

    // Return true if the player is near a merchant
...
// Checks if player is near monster, i.e, a monster is directly up, down, left, or right of the player. Returns true if so
bool Map::isNearMonster(){
    ...
    for (int i = 0; i < num_monsters; i++) {
        if (monsterPositions[i][0] == playerPosition[0] - 1 || monsterPositions[i][0] == playerPosition[0] + 1) {
            horizontalCheck = true;
        }
        if (monsterPositions[i][1] == playerPosition[1] - 1 || monsterPositions[i][1] == playerPosition[1] + 1) {
            verticalCheck = true;
        }
    }

    // Return true if the player is next to a monster
    if (horizontalCheck || verticalCheck) {
        return true;
    }
    else {
