...
// Main program driver. See beginning of file for pseudocode/algorithm.
int main() {
    // Declare variables
    ...
    Creature monster4("Tuxedo Plant", 25, blankInventory, 100, 100, 6, 6, 50, 100, 0);
    Creature monster5("Angry Gumball Machine", 25, blankInventory, 50, 50, 5, 5, 20, 10, 0);
    Creature monster6("General Arbiter", 500, blankInventory, 5000, 5000, 15, 15, 1000, 180, 0);
    ...
    Creature monster10("Emperor Reyalp", 999, blankInventory, 10000, 10000, 999, 999, 1200, 200, 0);
    ...
    while (userInput1 != 6 && mainPlayerParty.getPartyMemberAt(0).getHealth() > 0 && allMonsters[9].getHealth() > 0 && allMonsters[9].getPsyche() > 0) {
        ...
        // Allow the user to move until they wish to stop moving
        if (userInput1 == 1) {
            userCharInput = '1';
            while (userCharInput != '0') {
                mainMap.displayMap();
                cout << endl;
                mainMap.displayMoves();
                cout << "Which direction would you like to move? Input multiple characters to move several times in one go. Input 0 to back out." << endl;
                cin >> userCharInput;
        ...
        // If the player wishes to battle with a monster,
            ...
            // If the player is near a monster that is not the final boss, walk through a normal battle using the battle() function
                ...
                // If the battle resulted in the player defeating the monster, remove the monster from the map.
                    ...
                    int monsterNumber = mainMap.isNearMonster();
                    mainMap.setMonsterRowPosition(monsterNumber, -5);
                    mainMap.setMonsterColPosition(monsterNumber, -5);
                }
                cout << endl;
            }
        }
    ...
    else if (mainPlayerParty.getPartyMemberAt(0).getHealth() <= 0 && mainPlayerParty.getCurrentPartySize() == 1 && allMonsters[9].getCreatureName() != "Emperor Reyalp") {
    ...
    // and print the endGameStats.txt file
        ...
        cin.ignore();
        cout << "As you witness Young Reyalp's heart being poured in front of you, the surrounding code begins crashing down once more. The damage is irreparable, and there isn't much time left for any of you." << endl;
        cin.ignore();
        cout << "Then, a thought lights up in your head." << endl;
        cin.ignore();
        ...
        cout << "..." << endl;
        cin.ignore();
        cout << "\"ALRIGHT. EVERYONE TO THEIR STATIONS! THIS SHIP MAY GO DOWN, BUT WE'RE GOING TO MAKE SURE IT PUSHES THROUGH DETERMINED, TO THE VERY LAST MOMENT!\"" << endl;
        cin.ignore();
